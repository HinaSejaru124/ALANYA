...
public abstract class ChatApp extends Application {
    ...
    public void start(Stage primaryStage) throws IOException {
        ...
        // try {
        //     audioSetup = new AudioSetup(audioInfoSocket, this);
        //     audioSetup.start();
        //     new AudioSendThread(audioSocket, audioSetup).start();
        //     new AudioReceiveThread(audioSocket, audioSetup).start();
        // } catch (LineUnavailableException e) {
        //     showError("Impossible d'établir la communication audio: " + e.getMessage());
        // }
        // audioStatus = new PrintWriter(new OutputStreamWriter(audioInfoSocket.getOutputStream()),
        //         true);

        // // Threads pour les appels video
        // try {
        //     videoSetup = new VideoSetup(videoInfoSocket, this);
        //     videoSetup.start();
        //     new VideoSendThread(audioVideoSocket, videoSocket, videoSetup).start();
        //     new VideoReceiveThread(audioVideoSocket, videoSocket, videoSetup, this).start();
        // } catch (LineUnavailableException e) {
        //     showError("Impossible d'établir la communication vidéo: " + e.getMessage());
        // }
        // videoStatus = new PrintWriter(new OutputStreamWriter(videoInfoSocket.getOutputStream()),
        //         true);
        ...
        //     audioSetup.onCall = true;
        //     // addAudioCallBubble(true, audioSetup);
        //     audioStatus.println(true);
        // });

        // hangUpButton.setOnAction(e -> {
        //     audioSetup.onCall = false;
        //     audioStatus.println(false);
        // });

        // videoCallButton.setOnAction(e -> {
        //     videoSetup.onCall = true;
        //     videoStatus.println(true);
        // });
    ...
    public void addFile(String iconPath, String fileName, boolean isSentByUser) {
        ...
        fileBubble.setStyle("-fx-background-color: " + (isSentByUser ? "#F59C01" : "#FFFFFF") + ";"
        ...
        fileBubble.setOnMouseEntered(
                e -> fileBubble.setStyle("-fx-background-color: " + (isSentByUser ? "#BF7A01" : "#f5f5f5") + ";"
                        + "-fx-background-radius: 15; -fx-border-radius: 15;"));

        fileBubble.setOnMouseExited(
                e -> fileBubble.setStyle("-fx-background-color: " + (isSentByUser ? "#F59C01" : "#FFFFFF") + ";"
                        + "-fx-background-radius: 15; -fx-border-radius: 15;"));
    ...
    public void addAudioCallBubble(boolean isSentByUser, AudioSetup audioSetup) {
        ...
        callBubble.setStyle("-fx-background-color: " + (isSentByUser ? "#F59C01" : "#FFFFFF") + ";"
                + "-fx-background-radius: 15; -fx-border-radius: 15;");
    ...
    public void startVideoCall() {
        ...
        new Thread(() -> {
            while (true)
                Platform.runLater(() -> imageView.setImage(convertToFxImage(image)));
        }).start();
    }
    ...
    private void addContact(String name, String status, String imagePath) {
        HBox contact = new HBox(10);
        contact.setPadding(new Insets(10));
        contact.setStyle("-fx-background-color: white; -fx-cursor: hand;");

        // Effet de survol
        contact.setOnMouseEntered(e -> contact.setStyle("-fx-background-color: #f0f0f0; -fx-cursor: hand;"));
        contact.setOnMouseExited(e -> contact.setStyle("-fx-background-color: white; -fx-cursor: hand;"));
        ...
        contact.getChildren().addAll(profilePic, contactInfo);
